...
# Create second solid: a cylinder
cylinder = cq.Workplane("YZ").circle(100).extrude(-400)

print("Performing cut...",end="")
result = cylinder.cut(cavity)   # Union (Fuse)
print("done.")

# Export the resulting solid to a STEP file
cq.exporters.export(result, "boolean_result.step")

# Display result in the CadQuery Viewer (CQ-Editor)
try:
    from cq_viewer import show
    show(result)
except ImportError:
    print("cq_viewer is not installed. Install cq-editor or use an external viewer.")

print("STEP file 'boolean_result.stp' exported successfully!")
